Shader "Hidden/Locked/.poiyomi/Poiyomi Toon World/0312e4ae4b32b19479e3e3b306c6ed4f"
{
	Properties
	{
 ...
 #define OPTIMIZER_ENABLED 
			...
			void calculateRGBNormals(inout PoiMesh poiMesh, inout PoiMods poiMods)
			{
				...
				if (10.0 > 0 && 1.0)
				{
					nR = UnpackScaleNormal(POI2D_SAMPLER_PAN_STOCHASTIC(_RgbNormalR, _trilinear_repeat, poiUV(poiMesh.uv[0.0], PoiUVMerge(float4(1,1,0,0), float4(1,1,0,0) )), float4(0,0,0,0), 0.0), 10.0);
					hasR = 1;
				...
				if (0.0 > 0 && 0.0)
				{
					nG = UnpackScaleNormal(POI2D_SAMPLER_PAN_STOCHASTIC(_RgbNormalG, _trilinear_repeat, poiUV(poiMesh.uv[0.0], PoiUVMerge(float4(1,1,0,0), float4(1,1,0,0) )), float4(0,0,0,0), 0.0), 0.0);
					hasG = 1;
				...
				if (0.0 > 0 && 0.0)
				{
					nB = UnpackScaleNormal(POI2D_SAMPLER_PAN_STOCHASTIC(_RgbNormalB, _trilinear_repeat, poiUV(poiMesh.uv[0.0], PoiUVMerge(float4(1,1,0,0), float4(1,1,0,0))), float4(0,0,0,0), 0.0), 0.0);
					hasB = 1;
				...
				if (0.0 > 0 && 0.0)
				{
					float3 normalToBlendWith = UnpackScaleNormal(POI2D_SAMPLER_PAN_STOCHASTIC(_RgbNormalA, _trilinear_repeat, poiUV(poiMesh.uv[0.0], PoiUVMerge(float4(1,1,0,0), float4(1,1,0,0))), float4(0,0,0,0), 0.0), 0.0);
			...
			void applyMatcap(inout PoiFragData poiFragData, in PoiCam poiCam, in PoiMesh poiMesh, inout PoiLight poiLight, in PoiMods poiMods)
			{
				...
				matcap = UNITY_SAMPLE_TEX2D_SAMPLER_LOD(_Matcap, _trilinear_repeat, matcapUV, matcapSmoothness) * float4(poiThemeColor(poiMods, float4(0.3936214,0.3936214,0.3936214,1).rgb, 0.0), float4(0.3936214,0.3936214,0.3936214,1).a);
				#else
				matcap = float4(poiThemeColor(poiMods, float4(0.3936214,0.3936214,0.3936214,1).rgb, 0.0), float4(0.3936214,0.3936214,0.3936214,1).a);
				#endif
				matcap.rgb *= lerp(1, poiLight.directColor, 0.0);
				matcapIntensity = 0.72;
				#ifdef POI_AUDIOLINK
				if (matcapALD.matcapALEnabled > 0)
				{
				...
				blendMatcap(poiLight, poiFragData, poiMods, 0.0, 0.0, 0.677, 0.0, 0.0, 0.0, matcap, matcapMask, 0.0, 0.374, 1.0, 2.0, matcapALD);
				#endif
			}
			#endif
			#ifdef POI_STYLIZED_StylizedSpecular
			...
			{
				float nDotH = dot(lerp(poiMesh.normals[0], poiMesh.normals[1], 0.451), poiLight.halfDir);
				...
				if (0.0)
				{
					specMask1 += pow(specArea, exp2(lerp(11, 1, 0.189))) * 0.038;
					specMask2 += pow(specArea, exp2(lerp(11, 1, 0.0))) * 0.0;
				}
				else
				{
					specMask1 += poiEdgeNonLinear(specArea, (1.0 - pow(0.189, 5)), 0.141) * 0.038;
				...
				if (poiFragData.toggleVertexLights)
				{
					...
					for (int index = 0; index < 4; index++)
					{
						...
						if (0.0)
						{
							vSpecMask = pow(specArea, exp2(lerp(11, 1, 0.189))) * 0.038 * poiLight.vAttenuation[index];
							vSpecMask = max(vSpecMask, pow(specArea, exp2(lerp(11, 1, 0.0))) * 0.0 * poiLight.vAttenuation[index]);
						}
						else
						{
							vSpecMask = poiEdgeNonLinear(specArea, (1.0 - pow(0.189, 5)), 0.141) * 0.038 * poiLight.vAttenuation[index];
			...
			float4 frag(VertexOut i, uint facing : SV_IsFrontFace) : SV_Target
			{
				...
				float2 mainUV = poiUV(poiMesh.uv[0.0].xy, float4(3,3,0,0));
				if (0.0)
				{
				...
				#if defined(PROP_BUMPMAP) || !defined(OPTIMIZER_ENABLED)
				poiMesh.tangentSpaceNormal = UnpackScaleNormal(POI2D_SAMPLER_PAN_STOCHASTIC(_BumpMap, _MainTex, poiUV(poiMesh.uv[0.0].xy, float4(3,3,0,0)), float4(0,0,0,0), 1.0), 10.0);
				#else
				...
				poiFragData.baseColor *= poiThemeColor(poiMods, float4(0.6444244,0.4254881,1,1).rgb, 0.0);
				poiFragData.alpha = mainTexture.a * float4(0.6444244,0.4254881,1,1).a;
				#else
				...
				poiFragData.baseColor = lerp(poiFragData.baseColor, pow(abs(poiFragData.baseColor), 0.65), hueShiftAlpha.a);
